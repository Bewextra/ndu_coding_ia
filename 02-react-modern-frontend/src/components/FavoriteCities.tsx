...
const FavoriteCities = ({ favorites, onCityClick, currentCity }: FavoriteCitiesProps) => {
    ...
    <aside className="favorite-cities">
      ...
      <div className="favorite-cities-list">
        {favorites.map((city, index) => (
          <button
            key={index}
            className={`favorite-city-item ${currentCity === city.name ? 'active' : ''}`}
            onClick={() => handleCityClick(city.name)}
          >
            <div className="favorite-city-info">
              <span className="favorite-city-name">{city.name}</span>
              <span className="favorite-city-country">{city.country}</span>
            </div>
            <svg 
              className="favorite-city-icon"
              width="20" 
              height="20" 
              viewBox="0 0 24 24" 
              fill="currentColor"
